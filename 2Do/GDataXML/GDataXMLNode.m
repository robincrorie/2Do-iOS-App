...
{
	NSArray * elementsForName = [self elementsForName:name];
	
	if(elementsForName.count > 0) {
		GDataXMLElement * firstElement = elementsForName[0];
		return firstElement;
	} else {
		return nil;
	}
...
- (GDataXMLElement *)nodeForXPath:(NSString *)xpath error:(NSError **)error {
	NSArray * allNodes = [self nodesForXPath:xpath namespaces:nil error:error];
	
	GDataXMLElement * xmlElement;
	if (allNodes.count > 0) {
		xmlElement = allNodes[0];
		return xmlElement;
	} else {
		return nil;
	}
